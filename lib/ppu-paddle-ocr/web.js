...
`));
  ...
  async function be() {
    ...
    }) : o ? a(new WebAssembly.Instance(y, G()), y) : (ae ??= e.locateFile ? e.locateFile ? e.locateFile("ort-wasm-simd-threaded.jsep.wasm", m) : m + "ort-wasm-simd-threaded.jsep.wasm" : new URL("ort-wasm-simd-threaded.jsep.wasm", import.meta.url).href, l = await (async function(f) {
      var w = ae;
      if (!g && !z(w)) try {
  ...
  for (let i = 0; i < cropWidths.length; i++) {
    ...
    for (let d = 0; d <= CUT_SNAP_RANGE && !skipSpace; d++) {
      for (let cand of [ideal - d, ideal + d]) {
        let ch2 = chars[cand];
        if (cand > charIdx && cand < chars.length && ch2 !== void 0 && /\s/.test(ch2)) {
          cut = cand;
          skipSpace = true;
          break;
        }
...
function defaultWasmPaths() {
  let version = _e.versions.web ?? _e.versions.common;
...
function applyDefaultWasmPaths() {
  ...
  if (!inBrowser || _e.wasm.wasmPaths) return;
  _e.wasm.wasmPaths = defaultWasmPaths();
}
...
async function recognizeCropsBatched(crops, ctx, charactersDictionary) {
  ...
  let order = prepped.map((_, i) => i).sort((a, b) => {
    ...
    let wb2 = prepped[b]?.tensorWidth ?? 0;
    return wa - wb2;
  });
  ...
  jobs.forEach((job, i) => {
    ...
    if (job.cropWidths === null) {
      ...
      for (let i2 = 0; i2 < job.lineBoxes.length; i2++) {
        let lb2 = job.lineBoxes[i2];
        if (!lb2) continue;
        results.push({ text: (pieces[i2] ?? "").trim(), box: lb2.box, confidence: rec.confidence });
      }
    }
  });
  return sortByReadingOrder(results);
...
async function runCrossLineStrategy(sourceCanvas, validBoxes, ctx, charactersDictionary) {
  ...
  let resized = lineCrops.map(({ canvas, boxes, cropWidths }, i) => {
    ...
    let resizedWidth = Math.max(MIN_CROP_WIDTH, Math.round(targetHeight * ar2));
  ...
  for (let batch of batches) {
    ...
    for (let i = 0; i < batchSorted.length; i++) {
      ...
      for (let j2 = 0; j2 < item.boxes.length; j2++) {
        let lb2 = item.boxes[j2];
        if (!lb2) continue;
        let w = (item.cropWidths[j2] ?? 0) * scale;
        if (j2 === item.boxes.length - 1 && i < batchSorted.length - 1) w += SEPARATOR_GAP;
        flatSegments.push(w);
        flatBoxes.push(lb2);
      }
    }
    ...
    for (let k2 = 0; k2 < flatBoxes.length; k2++) {
      let lb2 = flatBoxes[k2];
      if (!lb2) continue;
      results.push({ text: (pieces[k2] ?? "").trim(), box: lb2.box, confidence: batchConf });
    }
  }
  return sortByReadingOrder(results);
}
...
// src/entry.js
try {
  _e.wasm.wasmPaths = new URL("./", import.meta.url).href;
  console.log("[Paddle] wasmPaths \u5DF2\u6539\u70BA\u672C\u6A5F:", _e.wasm.wasmPaths);
} catch (e) {
