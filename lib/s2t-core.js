...
function scanStream(u8, fontInfos) {
  ...
  const samples = new Map();          // family -> { x, y, size, n }
  let tmX = 0, tmY = 0, tmA = 1, lastWasTm = false;
  ...
  for (let ti = 0; ti < toks.length; ti++) {
    ...
    if (op === "Tm") {
      const n = operands.filter(o => o.t === "num");
      if (n.length >= 6) { tmA = n[0].v; tmX = n[4].v; tmY = n[5].v; lastWasTm = true; }
      operands = [];
      continue;
    }
    if (op === "Tf") {
    ...
    if (op === "Tj" || op === "TJ" || op === "'" || op === '"') {
      ...
      if (fi && fi.hasToUnicode && operands.length && curFontSize != null) {
        ...
        if (rec.items.length) {
          shows.push(rec);
          // 記低一個有代表性嘅樣本位置，畀 UI 喺報告度顯示原本字體嘅樣
          const hanN = rec.items.filter(it => it.kind === "glyph" && it.ch && isCJK(it.ch)).length;
          if (lastWasTm && hanN >= 2) {
            const eff = Math.abs(tmA) * curFontSize;
            const prev = samples.get(fi.family);
            const score = eff * Math.min(hanN, 6);
            if (!prev || score > prev.score) {
              samples.set(fi.family, { x: tmX, y: tmY, size: eff, n: hanN, score });
            }
          }
        }
    ...
    operands = [];
    if (op !== "Tm") lastWasTm = false;
  }
  return { units, shows, missingToUni, samples };
}
...
async function convert(arrayBuffer, opts) {
  ...
  for (let pi = 0; pi < pages.length; pi++) {
    ...
    for (const tgt of targets) {
      ...
      const { units, missingToUni, samples } = scanStream(bytes, fontInfos);
      missingToUni.forEach(x => stats.missingToUni.add(familyOf(x)));
      if (tgt.kind === "page") {
        for (const [fam, sm] of samples) {
          const prev = stats.samples.get(fam);
          if (!prev || sm.score > prev.score) stats.samples.set(fam, Object.assign({ page: pi }, sm));
        }
      }
...
/* 由字型名估款式（估得中就中，估唔中就靠報告嗰張樣本圖） */
function guessStyle(name) {
  const n = String(name || "");
  if (/KT|Kai|楷/i.test(n)) return "楷";
  if (/SS|Song|Sung|Ming|Serif|Batang|宋|明/i.test(n)) return "宋／明";
  if (/HT|Hei|Gothic|Sans|黑/i.test(n)) return "黑";
  if (/Y[0-9]|Yuan|Round|圓|圆|POP|綜藝|综艺/i.test(n)) return "圓／綜藝";
  return "";
}

root.S2T = {
  convert, convertSequence, tokenize, parseToUnicode,
  isCJK, familyOf, fontFlavour, patchFontkitSubset, guessStyle,
};
