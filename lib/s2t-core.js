 ...
 */
function convertSequence(chars, dict, overrides, variant) {
  const s = chars.join("");
  ...
  let i = 0;
  while (i < s.length) {
    if (overrides && overrides.has(s[i])) { out[i] = overrides.get(s[i]); i++; continue; }
    let hit = false;
    const lim = Math.min(maxp, s.length - i);
    for (let L = lim; L >= 2; L--) {
      const sub = s.substr(i, L);
      const t = PH[sub];
      if (t && t.length === L) {
        let blocked = false;
        if (overrides) for (let k = 0; k < L; k++) if (overrides.has(s[i + k])) blocked = true;
        if (!blocked) {
  ...
  if (VM) {
    for (let k = 0; k < out.length; k++) {
      if (out[k] !== s[k] && VM[out[k]]) out[k] = VM[out[k]];
    }
  }
  return out;
}
        ...
        for (const o of operands) {
          if (o.t === "str" || o.t === "hex") {
            ...
            for (let i = 0; i + cb <= o.v.length; i += cb) {
              ...
              rec.items.push(item);
              if (ch && isCJK(ch)) units.push({ rec, item, ch });
            }
...
async function convert(arrayBuffer, opts) {
  ...
  const stats = {
    totalCJK: 0, changed: 0, pagesTouched: 0,
  ...
  for (let pi = 0; pi < pages.length; pi++) {
    ...
    for (const tgt of targets) {
      ...
      stats.totalCJK += units.length;

      const src = units.map(u => u.ch);
      const dst = convertSequence(src, D, opts.overrides, variant);
      ...
      for (let i = 0; i < units.length; i++) {
        ...
        stats.pairs.set(src[i] + "→" + dst[i], (stats.pairs.get(src[i] + "→" + dst[i]) || 0) + 1);
        stats.neededChars.add(dst[i]);
        ...
        if (!hasGlyph(target, ch)) {
          const vm = D[variant] || {};
          const cands = [];
          for (const a of (D.alt && D.alt[src[i]]) || []) { cands.push(a); if (vm[a]) cands.push(vm[a]); }
          if (D.c[src[i]]) cands.push(D.c[src[i]]);
          cands.push(src[i]);   // 最後手段：保留簡體，好過空白
          const found = cands.find(a => a.length === 1 && a !== ch && hasGlyph(target, a));
          if (found) ch = found;
          else {
            stats.noGlyph.set(dst[i], (stats.noGlyph.get(dst[i]) || 0) + 1);
            continue;
          }
        }
